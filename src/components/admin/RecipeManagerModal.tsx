...
import { useItemRecipes, RecipeIngredient } from '@/hooks/useItemRecipes';
...
export const RecipeManagerModal: React.FC<RecipeManagerModalProps> = ({
  ...
  const { recipes, isLoading, addIngredient, updateIngredient, removeIngredient } = useItemRecipes(item?.id);
  ...
  // Filter out items that are already in the recipe and the item itself
  const availableIngredients = useMemo(() => {
    const usedIds = new Set(recipes.map(r => r.ingredient_item_id));
    return allItems.filter(i => i.id !== item?.id && !usedIds.has(i.id));
  }, [allItems, recipes, item]);
  ...
  const handleUpdateQuantity = async (recipe: RecipeIngredient, newMl: string, newUnits: string) => {
    const ml = newMl ? parseInt(newMl) : null;
    const units = parseInt(newUnits) || 1;
    await updateIngredient(recipe.id, ml, units);
  };
          ...
          <div className="flex-1 overflow-hidden">
            ...
            {isLoading ? (
              ...
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p className="text-xs">Adicione ingredientes acima</p>
              </div>
            ) : (
          ...
          {recipes.length > 0 && (
            <div className="p-3 bg-primary/10 rounded-lg text-sm">
              <p className="font-medium text-primary">
                Ao vender "{item.name}", o estoque será decrementado automaticamente:
              </p>
